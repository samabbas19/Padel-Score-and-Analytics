...
class PlayerKeypointsTracker(Tracker):
    ...
    def predict_sample(self, sample: Iterable[np.ndarray], **kwargs) -> list[PlayersKeypoints]:
        ...
        for result in results:
            ...
            if players_keypoints_detection.numel() == 0:
            ...
            if players_keypoints_detection.ndim == 2:
                ...
                players_keypoints_detection.ndim == 4
                and players_keypoints_detection.shape[0] == 1
            ):
                players_keypoints_detection = players_keypoints_detection.squeeze(0)

            if players_keypoints_detection.ndim != 3:
                raise ValueError(
                    "Expected player keypoints with shape "
                    f"(players, keypoints, xy), got {tuple(players_keypoints_detection.shape)}"
                )
