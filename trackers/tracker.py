...
class Tracker(ABC):
    ...
    def DEVICE(self) -> str:
        if torch.cuda.is_available():
            return "cuda"
        if os.environ.get("PADEL_STRICT_GPU", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }:
            raise RuntimeError("PADEL_STRICT_GPU=1 but torch.cuda.is_available() is False")
        return "cpu"
    ...
    def load_predictions(self) -> None:
        ...
        if self.load_path:
            ...
            if not load_path.exists():
                print(f"{self.__str__()}: No cached predictions at {load_path}")
